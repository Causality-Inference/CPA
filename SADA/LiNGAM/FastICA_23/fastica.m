...
% multidimensional signals(多维信号). Each row of matrix mixedsig is one
...
% matrix W（解混矩阵） and the corresponding mixing matrix A（混合矩阵）.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Check some basic requirements of the data%%%判断输入变量
if nargin == 0,%%%判断输入变量的个数
  error ('You must supply the mixed data as input argument.');
end

if length (size (mixedsig)) > 2,%%%输入数据是两个维度的，不能超过三个
  error ('Input data can not have more than two dimensions.');
end

if any (any (isnan (mixedsig))),%%%判断数据中是否存在NaN
  error ('Input data contains NaN''s.');
end

if ~isa (mixedsig, 'double')%%%%
%   fprintf ('Warning: converting input data into regular (double) precision.\n');
  mixedsig = double (mixedsig);%%%强制转换成double型
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Remove the mean and check the data

[mixedsig, mixedmean] = remmean(mixedsig);%%%去均值

[Dim, NumOfSampl] = size(mixedsig);%%%Dim维度，NumofSample样本数量

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Default values for optional parameters可选参数的默认值

% All
verbose           = 'on';%%%报告程序的执行过程
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Read the optional parameters选择参数的判断

if (rem(length(varargin),2)==1)%%%rem求余数，判断参数是否是pairs例子('approach','symm')
  error('Optional parameters should always go by pairs');
else
  for i=1:2:(length(varargin)-1)
    if ~ischar (varargin{i}),%%%非strings
    ...
    switch lower (varargin{i})
     ...
     case 'pcae'%%%已经知道协方差矩阵的特征值分解，直接给出参数，pcae,矩阵特征向量；pcad,矩阵特征值
     ...
     case 'pcad'
      ...
      jumpPCA = jumpPCA + 1;
      D = varargin{i+1};
     case 'whitesig'%%%已经知道白化数据，直接给出这三个参数，都是矩阵（whitesig白化信号;whitemat白化矩阵;dewhitemat去白化矩阵）
...
if only > 1%%%only=1,pca;only=2,white;only=3,all;
%   xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  = 1
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % Whitening the data
