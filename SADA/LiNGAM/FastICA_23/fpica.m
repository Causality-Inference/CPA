...
%
% whitesig         白化信号                     :the whitened data as row vectors
% whiteningMatrix 白化矩阵                      :can be obtained with function whitenv
% dewhiteningMatrix     去白化矩阵                :can be obtained with function whitenv
...
global g_FastICA_interrupt;%%%%%全局变量
...
if nargin < 3, error('Not enough arguments!'); end%%%%nargin判断输入变量的个数，至少有3个参数（whitesig ，按照行白化数据，）
[vectorSize, numSamples] = size(X);%%%%%vectorSize表示样本的维数，numSamples变量的个数
if nargin < 20, s_verbose = 'on'; end%%%%第20个参数，用来文本报告程序进度，默认是报告
if nargin < 19, displayInterval = 1; end%%%第19个参数，画图时的迭代数
if nargin < 18, displayMode = 'on'; end%%%画出独立的估计成分
if nargin < 17, sampleSize = 1; end%%%在一次迭代中使用的样本百分比
if nargin < 16, guess = 1; end%%%矩阵A的初始化操作，如果是初始是'rand'请忽略
if nargin < 15, initState = 'rand'; end%%%['rand'|'guess']，对矩阵A初始化操作
if nargin < 14, maxFinetune = 100; end%%%微调（fine-tuning）的最大迭代次数，默认值为100
if nargin < 13, maxNumIterations = 1000; end%%%最大迭代次数，默认值为1000
if nargin < 12, epsilon = 0.0001; end%%%终止的标准
if nargin < 11, stabilization = 'on'; end%%%决定程序是否执行稳定版本的算法，当为'on'时，程序遇到两点间被卡主，mu（这里是myy）的值减半，或者迭代次数达到一半时，算法未收敛，mu减半，执行完剩余部分
if nargin < 10, myy = 1; end%%%默认值为1，如果不是1，程序执行稳定版本的算法
if nargin < 9, a2 = 1; end
if nargin < 8, a1 = 1; end
if nargin < 7, finetune = 'off'; end%%%默认值'off'，对'g'进行微调
if nargin < 6, g = 'pow3'; end%%%默认值是'pow3',g(u)=u^3,也可以是'tanh',g(u)=tanh(a1*u);'gauss',g(u)=u*exp(-a2*u^2/2);'skew',g(u)=u^2
if nargin < 5, numOfIC = vectorSize; end     % vectorSize = Dim,默认为样本的维数
if nargin < 4, approach = 'defl'; end%%%去相关方法，默认'delf'(one-by-one),'sym'（in parallel,并行）,估计独立成分
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Checking the data

if ~isreal(X)%%%%如果X不是实数，输出数据中包含虚部
...
switch lower(s_verbose)%%%lower将字母变为小写
 case 'on'
  b_verbose = 1;
 case 'off'
  b_verbose = 0;
...
% if b_verbose, fprintf('Used approach [ %s ].\n', approach); end%%%如果b_verbose==1，即为'on'，则输出使用的方法
...
elseif sampleSize < 1
  if (sampleSize * numSamples) < 1000%%%如果样本足够多，就选择1000个作为一次迭代的个数，如果样本不足1000，则全部选择作为一次迭代的个数
...
switch lower(initState)%%%矩阵初始化操作
 case 'rand'
  initialStateMode = 0;
 case 'guess'
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Checking the value for display mode.%%%画图，主要是估计的独立成分
...
% The displayInterval can't be less than 1...%%%画图时的迭代数
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SYMMETRIC APPROACH对称方法
if approachMode == 1,%%%并行
  ...
  if initialStateMode == 0
    ...
    B = orth(rand(vectorSize, numOfIC) - .5);%%%orth返回矩阵的正交基
...
%       fprintf('Note that the plots are probably wrong.\n');%%%图形可能错误
	...
	A = [];
      ...
      return;%%%如果在函数文件中使用了return语句，则执行到该语句就结束函数的执行，程序流程转回到调用该函数的位置。也可以不使用return语句，被调用函数执行完成后自动返回。
    end
...
%         fprintf('\n\nCalculation interrupted by the user\n');
    ...
    % Symmetric orthogonalization.对称正交化
    B = B * real(inv(B' * B)^(1/2));
    
    % Test for termination condition（终止条件）. Note that we consider opposite
    % directions here as well.
    minAbsCos = min(abs(diag(B' * BOld)));
    minAbsCos2 = min(abs(diag(B' * BOld2)));
    
    if (1 - minAbsCos < epsilon)
      if finetuningEnabled & notFine%%%finetuningEnabled对g进行微调，notFine
...
%         if b_verbose, fprintf('Convergence after %d steps\n', round); end
    ...
    elseif stabilizationEnabled%%%稳定算法
