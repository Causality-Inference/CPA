function [cut_set,nodeA,nodeB,nodeCD]=CAPA(X,r_s)
nodeCD = [];
...
idxGloble = 1:n;
[~, nNode]=size(X);
    [cut_set,nodeA,nodeB,Pa,Pb,Pc]=CAPA_Split_Basic(X(:, 1:nNode),Ds_Temp,idxGloble(1:nNode));
end

function [cut_set,nodeA,nodeB,Pa,Pb,Pc]=CAPA_Split_Basic(X,Ds_Temp,idxNow)
[~, nNode]=size(X);
idxA=false(nNode, 1);
idxB=false(nNode, 1);
idxCut=true(nNode, 1);
