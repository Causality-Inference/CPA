function [skeleton] = stru_learn_part(data,Part,maxCset,Alg)
n = size(data,2);
skeleton = ones(n,n);
for c = 1:n
    skeleton(c,c) = 0;
end
skeletonB = skeleton; % for direction
for k = 0:maxCset
    for i = 1:n-1
        if ~ismember(i,Part)
            continue
        ...
        for j = i+1:n
            ...
            if skeleton(i,j) == 1 || skeleton(j,i) == 1
                if k == 0
...
%       skeleton = skeleton.*skeletonB;
%       for i = 1:n+1
%           for j = i+1:n
%               if skeletonB(i,j) == 0 && skeletonB(j,i) == 0
%                   [i,j]
%               end
%           end
%       end
