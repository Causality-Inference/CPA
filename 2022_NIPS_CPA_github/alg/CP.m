function [cut_set,nodeA,nodeB,nodeCD]=CP(X,r_s)
% r_s = find_rough_skeleton(X,2);
n = size(r_s,1);
Ds_Temp = ones(n,n)-r_s-eye(n,n);
idxGloble = 1:n;
[~, nNode]=size(X);
    [cut_set,nodeA,nodeB,nodeCD,Pa,Pb,Pc] = CP_Split_Basic(X(:, 1:nNode),Ds_Temp,idxGloble(1:nNode));
end

function [cut_set,nodeA,nodeB,nodeCD,Pa,Pb,Pc]=CP_Split_Basic(X,Ds_Temp,idxNow)
[~, nNode]=size(X);
idxA=false(nNode, 1);
idxB=false(nNode, 1);
idxCut=true(nNode, 1);
