...
def get_args():
    ...
    parser.add_argument('--data_variable_size', type=int, default=37,
...
def simulate_random_dag() -> nx.DiGraph:
    """Simulate random weighted DAG with some fixed skeleton.
...
def simulate_sem(G: nx.DiGraph,
    ...
    X = simulate_linear_sem_hz(W, n, noise_type)
    return X
...
# training:
#===================================
def train(args, train_loader, epoch, encoder, decoder, scheduler, ground_truth_G, lambda_A, c_A, optimizer, rel_rec, rel_send):
    t = time.time()
    nll_train = []
    kl_train = []
    mse_train = []
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        met = MetricsDAG(nx.DiGraph(graph), ground_truth_G)

        fdr, tpr, fpr, shd, nnz = met['fdr'], met['tpr'], met['fpr'], met['shd'], met['nnz']
...
def DAG_GNN_main():
    ...
    try:
        for step_k in range(k_max_iter):
            while c_A < 1e+20:
                for epoch in range(args.epochs):
                    ELBO_loss, NLL_loss, MSE_loss, graph, origin_A = train(args, train_loader, epoch, encoder, decoder, scheduler, ground_truth_G, lambda_A, c_A, optimizer, rel_rec, rel_send)
        ...
        graph[np.abs(graph) != 0 ] = 1

        # print(graph)
        met = MetricsDAG(nx.DiGraph(graph), ground_truth_G)
        met['time'] = (time.time() - t_total)
        return met   
