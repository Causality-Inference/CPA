...
def get_args():
    ...
    parser.add_argument('--graph_threshold', type=  float, default = 0.1,  # 0.3 is good, 0.2 is error prune
    ...
    parser.add_argument('--lambda_CI',  type = float, default= 1.0,
                        help='coefficient for CI constraint.')
    parser.add_argument('--c_A',  type = float, default= 1.0,
                        help='coefficient for absolute value h(A).')
    parser.add_argument('--hard_constraint',  type = bool, default= True,
                        help='use hard constraint or soft constraint.')
    ...
    parser.add_argument('--lr', type=float, default=5e-4,  # basline rate = 1e-3
    ...
    parser.add_argument('--k_max_iter', type = int, default = 2e2,
...
def simulate_sem(G: nx.DiGraph,
    ...
    X = simulate_linear_sem_hz(W, n, 'uniform')
    return X
...
# training:
#===================================
def train(args, train_loader, epoch, encoder, decoder, scheduler, best_val_loss, dataset, CI_table, ground_truth_G, lambda_A, c_A, optimizer, rel_rec, rel_send):
    t = time.time()
    nll_train = []
    kl_train = []
    mse_train = []
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        CI_loss = CI_test_Loss(origin_A, CI_table)
        
        # compute h(A)
        h_A = _h_A(origin_A, args.data_variable_size)
        
        # very important!!! Hard or soft?
        # Soft: loss += args.lambda_CI * CI_loss
        # Hard: h_A += args.lambda_CI * CI_loss
        if args.hard_constraint == True:
            h_A += args.lambda_CI * CI_loss 
        else:
...
# main
#===================================
def DAG_GNN_CIR_main(dataset, B_true, CI_table, hard_constraint, lambda_CI):
    ...
    args.lambda_CI = lambda_CI
    args.hard_constraint = hard_constraint
    # ================================================
        ...
        for step_k in range(k_max_iter):
            while c_A < 1e+20:
                for epoch in range(args.epochs):
                    ELBO_loss, NLL_loss, MSE_loss, graph, origin_A = train(args, train_loader, epoch, encoder, decoder, scheduler, best_ELBO_loss, dataset, CI_table, ground_truth_G, lambda_A, c_A, optimizer, rel_rec, rel_send)
...
if __name__ == '__main__':
    ...
    for seed in range(num_iters):
        ...
        CI_table = prepare_CI_table(dataset)

        st = time.time()
        # structure learning
        G = DAG_GNN_CIR_main(dataset, B_true, CI_table, False, 1.0)
        est = G.copy()
        est[np.abs(est) < 0.1] = 0 
        est[np.abs(est) != 0] = 1 
        # calculate accuracy
